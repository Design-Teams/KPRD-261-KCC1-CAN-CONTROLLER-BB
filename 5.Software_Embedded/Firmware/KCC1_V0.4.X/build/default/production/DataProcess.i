...
void Uart2_Data_Handler(void)
{
    ...
    if(memcmp(((char*)Uart2_array+1),",LP,",4) == 0)
    {
        ...
        NRF.ID = (uint16_t)(atol(token_buf[0]));
        NRF.Act_Load = (uint16_t)(atol(token_buf[1]));
        NRF.Load_Count = (uint16_t)(atol(token_buf[2]));
        NRF.Bat_Vtg = (uint16_t)(atol(token_buf[3]));
        NRF.Status = (uint8_t)(atol(token_buf[4]));
# 358 "DataProcess.c"
    }
}
